...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return generatePageMetadata({
    ...
    path: '',
    locale,
    keywords: getKeywordsForPage(locale, 'home'),
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
            <div>
              ...
              <div className="space-y-2 text-lg text-body">
                <p><strong>{t('visit.days.tuesday')}:</strong> {OPENING_HOURS.tuesday}</p>
                <p><strong>{t('visit.days.wednesday')}:</strong> {OPENING_HOURS.wednesday}</p>
                <p><strong>{t('visit.days.thursday')}:</strong> {OPENING_HOURS.thursday}</p>
                <p><strong>{t('visit.days.friday')}:</strong> {OPENING_HOURS.friday}</p>
                <p><strong>{t('visit.days.saturday')}:</strong> {OPENING_HOURS.saturday}</p>
                <p><strong>{t('visit.days.sunday')}:</strong> {OPENING_HOURS.sunday}</p>
                <p className="text-sm italic">{t('visit.days.monday')}</p>
              </div>
            </div>
            <div>
              <h3 className="text-2xl mb-4 font-bold">{t('visit.address')}</h3>
              <p className="text-lg mb-6 text-body whitespace-pre-line">
                {ADDRESS[locale as keyof typeof ADDRESS]}
              </p>
              <div className="space-y-4">
                <div className="flex flex-col gap-4 justify-center">
                  <a href={`mailto:${EMAIL}`} className="text-forest hover:text-grasse transition-colors font-medium">
                    {EMAIL}
                  </a>
